...
public class Customer : MonoBehaviour, IPooledObject
{
    ...
    public void UpdateRequest()
    {
        ...
        if (curRequest == 0)
        {
            ...
            if (request == 1)
            {
                ...
                if (customerManager.LineQuDic.ContainsKey(new Vector3(-1.65f, 0.5f, 2.75f)))
                {
            ...
            else
            {
                ...
                if (customerManager.LineQuDic.ContainsKey(new Vector3(-1.65f, 0.5f, 0.25f)))
                {
    ...
    public void SetCustomerDestination(Vector3 target, Vector3? lookAt = null)
    {
        ...
        if (Vector3.Distance(target, CustomerLineManager.Instance.endPoint) < 0.01f)
        {
            endOn = true;
        }
        else
